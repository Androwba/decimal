...
int add_int_part(s21_decimal *temp, int *ten_pow, double num) {
  s21_decimal first_num, second_num, res, ten_dec, mult_helper;
  set_null(&res);
  int pow_helper = 0;
  int stop_flag = 0;
  int ret_val = 1;
  ...
  while ((int)int_num != 0) {
    s21_from_int_to_decimal((int)fmod(int_num, 10.0), &first_num);
    if (pow_helper > 0) {
      if (!stop_flag) stop_flag = s21_mul(second_num, ten_dec, &second_num);
    }
    if (!stop_flag) stop_flag = s21_mul(first_num, second_num, &mult_helper);
    if (!stop_flag) stop_flag = s21_add(mult_helper, res, &res);
    pow_helper++;
    int_num /= 10;
  }
  while (ret_val != 0) {
